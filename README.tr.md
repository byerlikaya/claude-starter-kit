...
![Skiller](https://img.shields.io/badge/skiller-33-f59e0b?style=flat-square)
...
| **"Nasıl" bilgisi nerede** | Kural + yöntem her agent prompt'una kopyalanır → çoğalma & tutarsızlık | **Agent = ince tetik** (kim/ne zaman); yöntem tek yerde, bir **skill**'te yaşar (tek doğruluk kaynağı), 33 skill'e yayılır |
...
`adopt` gibi güncelleme de bir git deposu ister. Değişikliğin nereye ineceği artık bir seçim: ilk devir `kit-adopt-<zaman damgası>` inceleme dalı açar (ana hattın temiz kalır); `.claude/` gitignore'lu rutin bir güncelleme **mevcut** dalına uygulanır (ayrı dal boş olurdu); `.claude/` **izlenen** bir güncelleme sorar. `--here` ya da `--new-branch` ile zorla. Her hâlükârda değişiklik staged ve commit'siz — diff'i incele, sonra commit'le ya da reset'le.
...
- **33 skill** — "nasıl" sorusunun tek kaynağı, her alan için bir tane (tüm katalog aşağıda).
...
<details>
<summary><b>Tüm skill kataloğu</b> — 33 skill, her birinden üretilir</summary>
...
> **Profil budaması token kazandırmaz.** `--backend` (10 ajan, 29 skill), `--fullstack`'ten (11 ajan, 33 skill) yalnızca birkaç yüz token ucuz. Profili işin kapsamını daraltmak için seç.
