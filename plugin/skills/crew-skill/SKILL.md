...
`AGENT_TEMPLATE.md` states the contract for a component. Full installs carry it (refreshed on `update`); the
plugin edition does not. No gate routes to it, so this command is the route, and it ends in the gates.
