...
Read-only auditor, like [[security-expert-crew]]: the relevant expert (backend / database / frontend) makes the
...
- Fix owner: **backend-expert-crew** (logic/allocation/async) · **database-expert-crew** (query/index/schema) ·
  **frontend-expert-crew** (render/bundle/payload).
- A slow path that is also unbounded is a DoS surface → hand it to **security-expert-crew**.
