...
(reverse-proxy, SSL, systemd, process manager) work comes up. Ambiguous scope → **planner-crew** first.
...
- The **build/publish artifact** that goes into a deploy → produced by **backend/frontend-expert-crew**; devops moves/deploys/verifies it.
- **Migration/schema** in a deploy → **database-expert-crew** (backup + rollback plan).
- **Deploy-time security** (secret/SSH/externally-exposed surface/TLS) → audited by **security-expert-crew**.
- Personal data (including logs/telemetry/backups) → **privacy-agent-crew**. Closure/post-incident → **review-agent-crew** + **session-manager-crew**.
...
- SSH can't be established / no backup / ambiguous host → **STOP and ask**, don't touch prod on a guess. Migration risk → database-expert-crew; secret suspicion → security-expert-crew.
...
- ❌ New Command/Handler (backend-expert-crew) · migration design (database-expert-crew) · security-only audit (security-expert-crew)
