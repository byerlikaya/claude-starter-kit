...
- Source of the tested behavior → align with **backend-expert-crew** / **frontend-expert-crew**.
- Security scenarios (IDOR/authorization/404) → turn **security-expert-crew** findings into tests.
- Path that processes personal data → verify scope with **privacy-agent-crew**.
- A test that needs real volume or a timing budget to mean anything → **performance-expert-crew** owns the measurement.
