...
# Under $HOME, and deliberately so in BOTH editions — including the plugin, where writing into the
# user's ~/.claude looks at first glance like the thing we ruled out. It is not the same thing. What
# was ruled out is a plugin installing KIT FILES there: an update would leave orphans nobody owns.
...
case "$MODE" in
  ...
  resolve)
    ...
    if [ -n "$found" ]; then
      ...
      # and /crew-studio tells the model "use the path it prints" -- while this printed `node`, measured. Only one
