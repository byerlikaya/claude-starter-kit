...
1. **DoD kapısı:** `/simplify` uygulandı mı · testler yeşil mi (test-expert) · (.NET'te) `sonarqube-check` 0/0/0/0.
   Biri kırmızıysa **DUR**, neyin eksik olduğunu bildir; erteleme yok.
2. Temizse **commit-agent** (commit-message) staged diff'ten atomik Conventional Commit **önerir**.
