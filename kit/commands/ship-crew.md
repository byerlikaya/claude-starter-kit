...
1. **DoD gate:** was `/simplify` applied · are tests green (@agent-test-expert-crew) · (if SonarQube is used) `sonarqube-check` 0/0/0/0.
   If any one is red, **STOP**, report what is missing; no deferral.
2. If clean, @agent-commit-agent-crew (commit-message) **proposes** an atomic Conventional Commit from the staged diff.
