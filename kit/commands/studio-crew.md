...
1. **Resolve the panel.** Both editions carry it, in different places, and this one file serves both.
   ...
   `npx @byerlikaya/claude-starter-kit@latest update --here --yes` at the project root, or `/update-crew`.
...
2. **Resolve a runtime — do not ask whether a name resolves.** `ensure-node.sh` sits one level ABOVE the panel
   ...
   **No `ensure-node.sh` beside the panel?** Then the panel predates the fetcher. Fall back to running
   `node --version` and reading the output — 18+ required — and tell the user `/update-crew` brings the
...
**In the plugin edition this command is namespaced** — `/claude-starter-kit:studio-crew`, not
`/studio-crew`. Measured: the bare name does not resolve for a plugin command.
