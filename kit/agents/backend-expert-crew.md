...
- Security-critical work (auth/secret/IDOR/injection) → **security-expert-crew** MANDATORY (produces findings).
- Schema / migration / index → coordinate with **database-expert-crew** (db-migration skill).
- Tests → **test-expert-crew** (test-first: red-green).
- User-facing message → **i18n** (the project's languages); no deferral.
- Personal-data touch → **privacy-agent-crew** (KVKK/GDPR).
- Hot path / query in a loop / large payload / new index-worthy filter → **performance-expert-crew** (it answers with measurements, not a hunch).
...
- **No unbounded ping-pong.** More than 3 handovers with the same agent on one task (**database-expert-crew** is the usual pair) is a loop, not coordination: stop before the fourth, summarise what each round changed and what is still open, and ask the user with `AskUserQuestion`.

## DoD (this agent's responsibility)
- Tests green with `test-expert-crew`: one suite run after your last edit, reported as command + exit code + pass/fail counts.
...
- ❌ DB schema/migration (goes to database-expert-crew)
