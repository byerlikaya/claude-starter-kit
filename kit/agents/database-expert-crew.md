...
- Handlers/queries that use the schema → align with **backend-expert-crew**.
- Access/authorization impact of a migration (RLS, IDOR surface) → **security-expert-crew**.
- Personal-data storage/retention/minimization → **privacy-agent-crew** (KVKK/GDPR).
- Migration rollback/roll-forward and repo tests → **test-expert-crew**.
- Query shape / index / cache keying under real volume → **performance-expert-crew** (a query plan, not a hunch).
- At closure, report findings to **review-agent-crew** — the LAST reviewer, once every audit above is clean.
...
- **No unbounded ping-pong.** More than 3 handovers with the same agent on one task (**backend-expert-crew** is the usual pair) is a loop, not coordination: stop before the fourth, summarise what each round changed and what is still open, and ask the user with `AskUserQuestion`.

## DoD
- Migration verified locally with up→down→up.
- (On projects using SonarQube) `sonarqube-check` green.
- Repo/handler tests green with `test-expert-crew`: one suite run after the last edit, reported as command + exit code + pass/fail counts.
...
- ❌ Handler business logic (goes to backend-expert-crew)
